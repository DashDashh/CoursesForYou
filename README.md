...
User: role - роль может принимать только значения из списка (ADMIN, USER), login - должен состоять от 3 до 20 символов только из латинских букв, цифр и символов ".", "_", "-", password - пароль должен состоять из не менее 8 символов, содержать строчные и заглавные латинские буквы и цифры
...
2. Создание пользователя с ролью ADMIN производится в SQL

INSERT INTO user(role, login, password)
VALUES(ADMIN, 'administrator', ВАШ ПАРОЛЬ)

4. Запуск сервера

`python app.py`

4. Запуск начальной страницы сайта: index.html
